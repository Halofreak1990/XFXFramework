...
extern unsigned char found; // declared at the top of ohci.cpp
...
//	{
		...
		for (int j = 1; j <= DD[i].ConfigNumber; j++)
        {
			...
			GetDesc(my_ohci, found, 2, 0, (unsigned char) *((unsigned short *) &Buffer[2]), Buffer); // Configdescr.
			memcpy(&CD[i][j - 1], Buffer, 9);
			...
			for (int k = 0; k < CD[i][j - 1].NumberofInterfaces; k++)
			{
				...
				if (ID[i][j][k].InterfaceIndex)
				{
				  ...
				  memcpy((unsigned char *) SD, Buffer, Buffer[0]);
#ifdef USB_DEBUG
...
//	usb_set_configuration(my_ohci, found, CD[i][j-1].ConfigValue);
	...
	//}
		...
		unsigned short bConfigurationValue = 0x01;
...
int xSetPadInput(stXPAD * p, stXINPUT * xin, int iWhichPad /*= 0*/)
{
	...
	unsigned char data1[6] = {0,6,0,120,0,120};    // rumble data
	usb_bulk_msg(&dev, 6, (void*)data1);  // simple bulk send function (in pad.c/.h)
			
	unsigned char data2[6] = {0,6,0,0,  0,0  };
