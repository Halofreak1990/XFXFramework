...
int get_control_msg(ohci_t * ohci, 
					unsigned char Addr,
					unsigned char request,     // 0x06
					unsigned char requesttype, // 0x80
					...
					unsigned char *data )
{
	...
	cmd.index		= index;
	cmd.length		= ((unsigned short) size);


	xMmLockUnlockBufferPages( (unsigned int)&cmd, 0x8, 0);
	unsigned int real_pcmd = xMmGetPhysicalAddress( (unsigned int)&cmd);


	unsigned char Descriptors[256] = {0};
	xMmLockUnlockBufferPages( (unsigned int)Descriptors, 256, 0 );
	unsigned int real_pDescriptors = xMmGetPhysicalAddress( (unsigned int)Descriptors );


	//unsigned int uTD = (unsigned int)pTD;
	//uTD += (unsigned int)0x10;
	//uTD &= 0xfffffff0;

	s_Transferdescriptor *TD;
	TD = (s_Transferdescriptor *)(((unsigned int *)ED) + 20);
	unsigned int TDA = EDA + 80;
	unsigned int realTDA = xMmGetPhysicalAddress( (unsigned int)TDA );
...
/*
int usb_get_device_descriptor(ohci_t * ohci, unsigned char Addr, int size, void *buf)
{
	unsigned char type  = USB_DT_DEVICE;
	unsigned char index = 0;

	int ret = get_control_msg(ohci, Addr,
		            USB_REQ_GET_DESCRIPTOR, USB_DIR_IN,
					(type << 8) + index, 0, size, (unsigned char*)buf );
	return ret;
...
/******************************************************************************/
/*
int usb_get_hub_descriptor(ohci_t * ohci, unsigned char Addr, int size, void *buf)
{

	int ret = get_control_msg(ohci, Addr,
		            USB_REQ_GET_DESCRIPTOR, USB_DIR_IN | USB_RT_HUB,
					(USB_DT_HUB << 8), 0, size, (unsigned char*)buf );
...
int usb_set_configuration(ohci_t * ohci, unsigned char Addr, int configuration)
{
	int ret=0;
...
//#define USETW2(w,h,l) (((unsigned char*)(w))[0] = (unsigned char)(l), ((unsigned char*)(w))[1] = (unsigned char)(h))
unsigned short USETW2(unsigned short v)
{
	unsigned short a = 0;;
	a |= (0xff00&(v<<8));
	a |= (0x00ff&(v>>8));
...
#define UGETW(w) (*(unsigned short *)(w))
#define USETW(w,v) (*(unsigned short *)(w) = (v))
#define UGETDW(w) (*(unsigned int *)(w))
#define USETDW(w,v) (*(unsigned int *)(w) = (v))
...
//#define USETW2XXX(w,h,l) (((unsigned char*)(w))[0] = (unsigned char)(l), ((unsigned char*)(w))[1] = (unsigned char)(h))
...
int do_hub_work(usbd_device * dev)
{
	...
	for (port = 1; port <= nports; port++) 
	{
		...
		unsigned short status = ps.wPortStatus;
		unsigned short change = ps.wPortChange;
					...
					unsigned char *data )
{
	...
	MmLockUnlockBufferPages( (unsigned int)&cmd, 0x8, 0);
	unsigned int real_pcmd = MmGetPhysicalAddress( (unsigned int)&cmd);
...
////////////////////////////////////////////////////////////////////////
	...
	//unsigned int format = 0x00401801;
	unsigned int format = 0x00401800;
	ED[1].Format = format;
